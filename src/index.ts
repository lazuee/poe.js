...
export class Poe {
	...
	async initialize() {
		...
		if (!this.#chat_data || !this.#viewer) {
			...
			throw new Error(`Poe API has changed! Please update the ${missingDataKey}.`);
		}
	...
	#getDeviceID() {
		...
		const device_ids = existsSync(device_id_path) ? JSON.parse(readFileSync(device_id_path, "utf8")) : {};
	...
	async #send_query(queryName: string, variables: Record<string, any>, queryDisplayName?: string) {
		for (let i = 0; i < this.#max_retries; i++) {
			...
			if (!("data" in result.data)) {
				this.#logger.warn?.(`'${queryName}' returns no data | Retrying (${i + 1}/${this.#max_retries})...`);
	...
	async #onMessage(data: WebSocket.RawData) {
		try {
			...
			for (const message_str of messages) {
				...
				if (message.suggestedReplies && Array.isArray(message.suggestedReplies)) {
				...
				for (const [key, value] of Object.entries(copiedDict)) {
					if (value === message.messageId && this.message_queues[key]) {
	...
	async send_message(prompt: Prompt, options?: { withChatBreak?: boolean; timeout?: number; onRunning?: () => Promisable<any>; onTyping?: (message: Message) => Promisable<any> }) {
		...
		const result = await this.#queue.add(async () => {
			...
			if (!humanMessage) throw new Error(`Daily limit reached for ${this.#display_name}.`);

			let humanMessageId = humanMessage?.node?.messageId ?? null;
			if (!humanMessageId) throw new Error(`Something went wrong while sending the message to ${this.#display_name}.`);
	...
	async delete(...message_ids: (number | number[])[]) {
		try {
			...
			await this.#send_query("DeleteMessageMutation", { messageIds: ids });
